...
# Install path: ~/.local/bin is the XDG-canonical user bin dir and
# lands in $PATH without intervention on most distros. Override with
# FRESHY_BIN_DIR=<path> if you'd rather another location.
FRESHY_BIN_DIR="${FRESHY_BIN_DIR:-$HOME/.local/bin}"
...
mkdir -p "$FRESHY_BIN_DIR"
if [[ -x "$work/freshy-bin" ]]; then
  install -m 0755 "$work/freshy-bin" "$FRESHY_BIN_DIR/freshy"
  echo "==> installed $FRESHY_BIN_DIR/freshy"
else
...
mkdir -p \
  ...
  "$HOME/.local/share/freshy/installers" \
...
"$FRESHY_BIN_DIR/freshy" init || true
unset FRESHY_SOURCE
...
The freshy binary lives at: $FRESHY_BIN_DIR/freshy
systemd timer is enabled and will run on boot, then every
